...
2. Unless the user or harness already chose the workspace, create a linked worktree under `.worktrees/` by default, or the path specified by the prompt or `AGENTS.md`. Verify the directory is ignored with `git check-ignore -q <directory>`. If not, add it to `.git/info/exclude` for local-only isolation; modify and commit `.gitignore` only when the user or repository instructions require a shared convention. Then create the worktree with `git worktree add "$path" -b "$branch"`. If the environment prevents worktree creation, report that limitation and work in place on a non-base branch.
3. Install dependencies using repository instructions and run the relevant baseline tests. If baseline fails, report the exact failure and ask whether to proceed. If that question returns `[Never-Ask]`, continue and keep the failure classified as pre-existing.
